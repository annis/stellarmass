...
def haloStellarMass ( filename="vt-stellar-masses.txt", outfile="vt-halos-stellar-masses.txt", verbose=1 ) :
    ...
    halos = np.sort(halos)
    if verbose: print "# hostid  median(ra) median(dec) median(z) ngals log(stellar_mass) std haloGal(log(smass)) (h=0.7, Om=0.3, flat)"
    fd.write("# hostid  median(ra) median(dec) median(z) ngals log(stellar_mass) haloGal(log(smass)) (h=0.7, Om=0.3, flat)\n")
    for halo in halos:
        ix = np.nonzero(hostid == halo)
        zmedian = np.median(z[ix])
        ramedian = np.median(ra[ix])
        decmedian = np.median(dec[ix])
        halogalMass = mass[ix].max()
        ngals = ra[ix].size
        ...
        if verbose: 
            print "{:10d} {:10.6f} {:10.5f}  {:6.3f}    {:4d}      {:6.3f} {:6.4f} {:6.3f}".format(
            halo,ramedian, decmedian, zmedian, ngals, sum_mass, sum_mass_std, halogalMass)
        fd.write("{:10d} {:10.6f} {:10.5f}  {:6.3f}     {:4d}      {:6.3f} {:6.4f} {:6.3f}\n".format(
            halo,ramedian, decmedian, zmedian, ngals, sum_mass, sum_mass_std, halogalMass))
    fd.close()
